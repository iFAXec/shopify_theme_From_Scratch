class SideCartItem extends HTMLElement {
  constructor() {
    ...
    this.querySelector(".cart-item-delete").addEventListener(
      "click",
      this.clearLineItem.bind(this)
    );
    this.querySelector(".cart-item-qty-plus").addEventListener(
      "click",
      this.addQty.bind(this)
    );
    this.querySelector(".cart-item-qty-minus").addEventListener(
      "click",
      this.minusQty.bind(this)
    );
  }
...
class SideCart extends HTMLElement {
    ...
    element.innerHTML = element.innerHTML
      .replace(
        ...
        item.featured_image.url
          ? item.featured_image.url + "&width=100"
  ...
  buildCart(cart) {
    ...
    if (cart.items.length === 0) {
      this.querySelector(
        "#cart-items"
      ).innerHTML = `<p>No items in your cart.</p>`;
    } else {
  ...
  addLineItem(line_item) {
    if (this.cart) {
      ...
      const existingLineItem = this.cart.items.find(
        (item) => item.id == line_item.id
      );
